...
date_spine as (
    ...
    select dateadd('day', seq4(), '{{ var("gaming1_start_date") }}'::date) as date_day
    from table(generator(rowcount => 1000))
    where dateadd('day', seq4(), '{{ var("gaming1_start_date") }}'::date) <= sysdate()
...
user_activity as (
    ...
    select *
    from {{ ref('agg_gaming1__user_activity_daily') }}
...
entries as (
    ...
    select *
    from {{ ref('agg_gaming1__entry_metrics_daily') }}
...
contests as (
    ...
    select *
    from {{ ref('agg_gaming1__contest_metrics_daily') }}
